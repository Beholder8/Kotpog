...
platform_filename = './images/trunk.png'
...
succes_sound = pygame.mixer.Sound('./sounds/success.wav')
music_sound = pygame.mixer.Sound('./sounds/music.wav')
frog_sound = pygame.mixer.Sound('./sounds/frog.wav')
...
def frogArrived(frog,arrived,game):
    if frog.position[0] > 33 and frog.position[0] < 53:
        position_init = [43,7]
        createArrived(frog,arrived,game,position_init)
...
def createArrived(frog,arrived,game,position_init):
    frog_arrived = Object(position_init,sprite_arrived)
    arrived.append(frog_arrived)
    frog_sound.play()
...
def nextLevel(arrived,enemys,platforms,frog,game):
    if len(arrived) == 5:
        arrived[:] = []
        frog.setPositionToInitialPosition()
        game.incLevel()
...
while True:
    ...
    while frog.lives > 0:
        ...
        nextLevel(arrived,enemys,platforms,frog,game)
